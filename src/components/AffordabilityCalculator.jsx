  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        {showAdvanced && (
          <div className="mt-3 grid grid-cols-1 gap-4 rounded-lg bg-slate-50 p-4 ring-1 ring-slate-200 md:grid-cols-3">
            <Field
      ...
      {result && (
        <div className="rounded-xl bg-emerald-50 p-6 ring-1 ring-emerald-200">
          <p className="text-sm font-medium text-emerald-800">Estimated maximum property price</p>
          <p className="mt-1 text-3xl font-bold text-emerald-900">{formatCurrency(result.maxPropertyPrice)}</p>
          <p className="mt-1 text-sm text-emerald-700">
            Based on a max loan of {formatCurrency(result.recommendedMaxLoan)} plus your{' '}
            {formatCurrency(inputs.deposit || 0)} deposit.
          </p>

          <div className="mt-4 grid grid-cols-1 gap-4 border-t border-emerald-200 pt-4 sm:grid-cols-2">
            <div>
              <p className="text-xs font-semibold uppercase text-emerald-700">Income multiple cap</p>
              <p className="text-lg font-semibold text-emerald-900">
                {formatCurrency(result.maxLoanByIncomeMultiple)}
              </p>
              <p className="text-xs text-emerald-700">{inputs.incomeMultiple || DEFAULT_INCOME_MULTIPLE}x annual income</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase text-emerald-700">Stress-tested affordability</p>
              <p className="text-lg font-semibold text-emerald-900">
                {formatCurrency(result.maxLoanByAffordability)}
              </p>
              <p className="text-xs text-emerald-700">at {result.stressRate.toFixed(2)}% (rate + stress margin)</p>
            </div>
          </div>
